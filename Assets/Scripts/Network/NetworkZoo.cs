...
public class NetworkZoo : MonoBehaviourPunCallbacks
{
    ...
    public string JoinRoomName = "Aula 1";
    ...
    public override void OnJoinedRoom()
    {
        ...
        GameObject player = PhotonNetwork.Instantiate(RemotePlayerObjectName, new Vector3(0, 0, 0), Quaternion.identity, 0); ;
        BNG.NetworkPlayer np = player.GetComponent<BNG.NetworkPlayer>();
        NetworkPlayerController npc = player.GetComponent<NetworkPlayerController>();
        if (np)
        {
            np.transform.name = "Avatar";
            //np.transform.name = "MyRemotePlayer";
            np.AssignPlayerObjects();
            npc.AssignPlayerControllers();
        }
    }
